    ...
    if age is not None:
        ...
        if age >= 65 or age <= 0:
            return build_validation_result(
                False,
                "age",
                "You should be worrying about your retirement before the age of retirement, "
                "please provide a different age.",
            )

    # Validate the investment amount, it should be > 0
    if investmentAmount is not None:
        investment = parse_int(
            investmentAmount
        )  # Since parameters are strings it's important to cast values
        if investment <= 4999.99:
            return build_validation_result(
                False,
                "investmentAmount",
                "We do not advise investors on portfolios smaller than $5000, "
...
def recommend_portfolio(intent_request):
    ...
    if risk_level == 'None':
        ...
        initial_recommendation += '70% bonds (AGG), 30% equities (SPY)'
